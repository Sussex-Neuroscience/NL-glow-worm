...
void loop() {
  ...
  int trials = 5;
  const int led_array[] = {4,5};
  int led = led_array[random(0,2)];
  const int brightness[] = {1,10,100,1000,4000};
  ...
  tlc.write();
          ...
          tlc.setPWM(led,bright);//the board starts counting leds from 0
          tlc.write();
          delay(1000*20);
          Serial.print(led);
          Serial.print(";");
          ...
          Serial.println(micros());
                ...
                if (led == 5) {
                          tlc.setPWM(4,bright);//the board starts counting leds from 0
                          tlc.write();
                           tlc.setPWM(led,0);
                           tlc.write();
                           delay(1000*20);
                            ...
                            tlc.setPWM(5,bright);//the board starts counting leds from 0
                            tlc.write();
                             tlc.setPWM(led,0);
                             tlc.write();
                             delay(1000*20);                            
